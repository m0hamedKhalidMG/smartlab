...
// GET /api/reports/summary — إحصائيات عامة مع فلتر تاريخ (admin/tech فقط)
router.get('/summary', authenticate, authorize('admin', 'technician'), (req, res) => {
  const { from, to } = req.query;
  // استخدام parameterized بدل string interpolation لتجنب SQL injection
  const dateParams = [];
  ...
  // أكثر الأجهزة أعطالاً — Prefix i.created_at لتجنب الغموض في JOIN
  ...
  // بيانات Bar Chart (الأعطال حسب الشهر)
...
// GET /api/reports/maintenance — سجل الصيانة مع فلتر
